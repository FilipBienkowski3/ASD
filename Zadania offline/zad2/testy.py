...
def internal_runtests(copyarg, printarg, printhint, printsol, check, generate_tests, all_tests, f, ACC_TIME):
    ...
    if SAVE_TESTS:
        with open("STORED_TESTS.py", "w") as file:
    ...
    for i, d in enumerate(TESTS):
        ...
        printhint(hint)
        try:
            if TIMER and os.name != 'nt':  # Only set signal timer on Unix systems
    ...
    if PRINT_STATUS:
        print_err(sys.argv[0].replace("_", " ").replace(".py", ""), passed, total, "%.2f" % total_time, status_line)
